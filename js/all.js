  ...
  for (let i = 0; i < 92; i++) {
    ...
    if (i < 46) {
      $(".card")
        .eq(i)
        .find(".card-front")
        .css("background-image", `url(./img/A${num}.png)`);
      $(".card").eq(i).attr("data-card", num);
    } else {
      $(".card")
        .eq(i)
        .find(".card-front")
        .css("background-image", `url(./img/B${num}.png)`);
      $(".card").eq(i).attr("data-card", num);
    }
    // 打散 92張卡的隨機索引(0-91)
    const target = Math.round(Math.random() * 91);
    // 隨機牌加到後方
    $(".card").eq(target).insertAfter($(".card").eq(i));
  }

  // 1-46隨機產28個數(要拿掉31組 留下15組)
  let str = "";
  let numbers = [];
  for (let i = 0; i < 31; i++) {
    str = Math.ceil(Math.random() * 46);
  ...
  $("#game").on("click", ".card", function () {
    ...
    if ($(".card-open").length === 2) {
      ...
      if (
        ...
        // 翻牌成功就顯示翻出的牌與對應的片假名
        let openNum = $(".card-open").attr("data-card");
        $("#review").html(
          `<img src="./img/C${openNum}.png"><div class="title">平假名</div><img src="./img/A${openNum}.png"><div class="title">羅馬拼音</div><img src="./img/B${openNum}.png">`
        );
      }
      // 1秒後 不管一不一樣都翻回來
      setTimeout(() => {
